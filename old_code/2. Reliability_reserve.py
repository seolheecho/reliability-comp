...
def RGTEP_CEC_model(transformation):
    ...
    m.cap_opt = pyo.Var(m.node, m.generator, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Operating capacity')
    m.rev_cap = pyo.Var(m.node, m.generator, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Reserve capacity')
    m.pnd= pyo.Var(m.node, m.generator, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Power produced and used')
    
    m.ppd = pyo.Var(m.node, m.generator, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Power produced')
    m.pexp = pyo.Var(m.node, m.generator, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Power exported')
    m.flow = pyo.Var(m.line, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Power flow')
    m.UD = pyo.Var(m.node, m.year, m.rpdn, m.sub, within=pyo.NonNegativeReals, doc='Unmet demand')
    ...
    def operating_capacity(m, i, k, t, n , b):
        return m.cap_ava[i,k,t] == m.cap_opt[i,k,t,n,b] + m.rev_cap[i,k,t,n,b]
    ...
    def reserve_capacity_limit(m, i, t, n , b):
        return sum(m.rev_cap[i,k,t,n,b] for k in m.generator) >= m.reserve_ratio * m.load_demand[i,t,n,b]
    ...
    def nodal_power_balance(m, i, t, n, b):
        return sum(m.pnd[i,k,t,n,b] for k in m.generator) + sum((1 - m.loss_line[l]) * m.flow[l,t,n,b] for l in m.line_to_node[i]) >= m.load_demand[i,t,n,b]
    # m.nodel_power_balance.pprint()            
    
    @m.Constraint(m.node, m.year, m.rpdn, m.sub)
    def power_balance_dpt(m, i, t, n, b):
        return sum(m.ppd[i,k,t,n,b] for k in m.generator) == sum(m.pnd[i,k,t,n,b] for k in m.generator) + sum(m.pexp[i,k,t,n,b] for k in m.generator)
    # m.power_balance_dpt.pprint()     
  
 
    @m.Constraint(m.node, m.year, m.rpdn, m.sub)
    def power_flow_export(m, i, t, n, b):
        return sum(m.pexp[i,k,t,n,b] for k in m.generator) == sum(m.flow[l,t,n,b] for l in m.line_fr_node[i])
...
for i in m.node:
    for k in m.generator:
        for t in m.year:
            for n in m.rpdn:
                for b in m.sub:
                    print("node", i, "plant", k, "year", t,
                        "| Available Cap", round(m.cap_ava[i,k,t].value,1), 
                        "day", n, "subperiod", b, "| E. produced", round(m.ppd[i,k,t,n,b].value,1), "| E. used for demand", round(m.pnd[i,k,t,n,b].value,1), 
                        "| E. exported", round(m.pexp[i,k,t,n,b].value,1))
